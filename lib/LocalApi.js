...
class LocalApi {
	...
	async waitUntilPosition() {
		...
		var currPos = result.currentPosition;
	 ...
	 * @returns {Promise} Promise object representing the result of the API call.
	 */
	async setPos(requestedPosition) {
		...
		var payload = {
			...
			'closedPosition': this.closedPosition,
			'suggestToCalibrate': ((currPos < this.openPosition) || (currPos > this.closedPosition) || (currPos !== requestedPosition))
		};
		return new Promise((resolve, reject) => {
			...
			} else if (currPos === requestedPosition) {
				resolve(payload);
			} else {
				this.request('/rpc/Slide.SetPos', { 'pos': requestedPosition })
					.then(() => { return this.waitUntilPosition(); })
					.then((json) => {
						payload.currentPosition = json.currentPosition;
						payload.suggestToCalibrate = ((payload.currentPosition < this.openPosition) || (payload.currentPosition > this.closedPosition) || (payload.currentPosition !== requestedPosition));
						resolve(payload);
					})
					.catch(e => reject(e));
			}
